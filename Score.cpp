...
void Score::ScoreBoard(int body, int max_body, int grow, int poison, int gate) {
...
bool Score::MissionBoard(int max_body, int grow, int poison, int gate) {
    ...
    if (gate >= gate_mission)
        mvwprintw(mission, 6, 3, " O : %d (Clear!)", gate_mission);
    else {
        mvwprintw(mission, 6, 3, " O : %d  ( X )  ", gate_mission);
        clear = false;
    }
